        ...
        foreach ($_FILES['files']['tmp_name'] as $key => $tmp_name) {
            ...
            if ($upload_file_extension == "pdf") {
                ...
                if (move_uploaded_file($upload_file_tmp_name, $uploadDirectory. $upload_file_name)) {
                    ...
                    $command = "java -jar /var/www/CAT_201_APACHE/pdf2txt.jar $convert_path $output_path";
                    $output = [];
                    $returnCode = 0;
            ...
            // check if the uploaded file is an TXT file
                ...
                if (move_uploaded_file($upload_file_tmp_name, $uploadDirectory . $upload_file_name)) {
                    ...
                    $command = "java -jar /var/www/CAT_201_APACHE/txt2pdf.jar $convert_path $output_path";
                    $output = [];
                    $returnCode = 0;
                    exec($command, $output, $returnCode);
